...
- Регистрация двухшаговая: `/api/auth/register` отправляет код на email, затем подтверждение через `/api/auth/verify`.
- Токены хранятся в httpOnly cookies `FG_AUTH/FG_REFRESH` (после `login`/`login/otp`/`verify`); фронт их не читает. В `sessionStorage` сохраняется только email для отображения.
